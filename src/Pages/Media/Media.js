...
const Media = () => {
  ...
  const { data: posts = [], refetch } = useQuery({
    queryKey: ['posts'],
    queryFn: async () => {
      const res = await fetch(`https://social-server-sandy.vercel.app/allPost`, {
        headers: {
                  ...
                  <>
                    {
                      like === false ?
                        ...
                        <>
                          <p className='my-auto text-sm mr-3'>{post?.like?.n}</p>
                          <button className='text-3xl' onClick={() => handleLikeReactAdd(post?._id)}><FaThumbsUp className='text-blue-600'></FaThumbsUp></button>
                        </>
                    }
                  </>
              }
